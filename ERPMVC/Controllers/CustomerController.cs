...
namespace ERPMVC.Controllers
{
    ...
    public class CustomerController : Controller
    {
        ...
        //Compras del cliente
        [HttpGet("[action]")]
        public async Task<ActionResult> GetPurchaseByCustomerId([DataSourceRequest]DataSourceRequest request, int CustomerId)
        {
            List<Contrato> _ContratoAndFactura = new List<Contrato>();
            try
            {
                string baseadress = config.Value.urlbase;
                HttpClient _client = new HttpClient();
                _client.DefaultRequestHeaders.Add("Authorization", "Bearer " + HttpContext.Session.GetString("token"));
                var result = await _client.GetAsync(baseadress + "api/Customer/GetPurchaseByCustomerId/" + CustomerId);
                string valorrespuesta = "";
                if (result.IsSuccessStatusCode)
                {
                    valorrespuesta = await (result.Content.ReadAsStringAsync());
                    _ContratoAndFactura = JsonConvert.DeserializeObject<List<Contrato>>(valorrespuesta);
                }
            }
            catch (Exception ex)
            {
            ...
            return Json(_ContratoAndFactura.ToDataSourceResult(request));
        }
